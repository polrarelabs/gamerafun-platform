...
const ModalShare = ({ open, setOpen }: PropsModalShare) => {
  ...
  const handleCopy = () => {
    try {
      navigator.clipboard.writeText(YOUR_URL);
    } catch (error) {
  ...
  return (
    <Modal
      ...
      sx={{
        backgroundColor: palette.colorModalShare?.bgColor,
        ...
        >
            ...
            {listIconShare.map((item, index) => (
              ...
              >
                <Stack
                  sx={{
                    borderRadius: "50%",
                    backgroundColor: palette.colorModalShare?.bgColor1,
                    width: "56px",
                    height: "56px",
                    display: "flex",
                    justifyContent: "center",
          ...
          >
            <Stack direction={"column"} spacing={1}>
              ...
              <Text variant={"subtitle2"}>{YOUR_URL}</Text>
            </Stack>
